...
def read_queues_from_hana(config: Config, logger: logging.Logger) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info('Leyendo colas desde "%s"."%s"...', config.hana_schema, config.hana_colas_table)
...
def main() -> int:
    ...
    try:
        ...
        log_params(logger, ["GENESYS_CLIENT_ID", "GENESYS_CLIENT_SECRET", "GENESYS_REGION", "HPR_HOST", "HPR_PORT", "HPR_USER", "HPR_PASSWORD", "HANA_SCHEMA", "HANA_COLAS_TABLE", "HANA_VOLUMEN_TABLE", "RUN_MODE", "DATE", "START_DATE", "END_DATE", "START_LOCAL", "END_LOCAL", "DAYS_BACK", "DRY_RUN"])
