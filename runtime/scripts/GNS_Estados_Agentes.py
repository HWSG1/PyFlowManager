...
PYFLOW_PARAMS = {
    ...
    "HPR_USER": {
        "type": "global",
        "global_key": "HPR_SERVICE_USER",
        "label": "SAP HANA Service User",
        "required": True
    },
    "HPR_PASSWORD": {
        "type": "global",
        "global_key": "HPR_SERVICE_PASSWORD",
...
def main() -> int:
    ...
    try:
        ...
        log(f"[INFO] Usuarios leídos desde HANA: {len(user_ids)}")
        if not user_ids:
            log("[WARN] No se encontraron usuarios en HANA. Finalizando.")
            return 0
